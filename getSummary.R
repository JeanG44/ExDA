getSummary <- function(data, save_file=FALSE, file_dir=".", K=5, sepvalue=F){
  library(dplyr)
  
  ## functions that are used for summarization of data
  # 변수 타입 파악(로지컬일 경우 카테고리컬로 봄)
  # classes <- lapply(sapply(data, class),function(x) paste(x, collapse=""))
  classes <- sapply(data, class)
  classes <- ifelse(classes=='Date' | grepl("POSIX", classes), 'Time',
  ...
  if(save_file){
    if(NumericExists){
      write.csv(NumericVariables, paste(file_dir,'/Numeric.csv', sep=''), row.names=FALSE)
    }
    if(CategoricalExists){
      write.csv(CategoricalVariables, paste(file_dir,'/Categorical.csv', sep=''), row.names=FALSE)
    }
    if(TimeExists){
      write.csv(TimeVariables, paste(file_dir,'/Time.csv', sep=''), row.names=FALSE)
    }
  }
